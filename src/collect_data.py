...
def get_job_table(pr):
    df = pr.job_table()
    df = df[(df.status != "running") & (df.status != "submitted")]
...
class CollectData(PythonTemplateJob):
    def run_static(self):
        data_dict = {"fcc": defaultdict(list), "bcc": defaultdict(list)}
        df = get_job_table(self.project)
        ...
        for cs in ["fcc", "bcc"]:
            data_dict[cs]["structure"] = self.project.create.structure.atoms(
                elements=len(data_dict[cs]["positions"][0]) * ["Fe"],
                positions=data_dict[cs]["positions"][0],
                pbc=True,
                cell=data_dict[cs]["cell"][0],
            )
            x = np.einsum(
                "nji,nmj->nmi",
        ...
        self.output.data_dict = data_dict
